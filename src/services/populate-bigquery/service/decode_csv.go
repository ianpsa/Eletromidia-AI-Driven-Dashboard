...
func LoadCsvIntoBigQuery(dataset *bigquery.Dataset, file *storage.ObjectHandle, ctx context.Context) error {
	...
	if _, err := csvReader.Read(); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao ler header do csv: %w", err)
	}
...
IteratingCsvLines:
	...
	if err := ageInserter.Put(ctx, AgeRows); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao inserir age no BigQuery: %w", err)
	}

	if err := genderInserter.Put(ctx, GenderRows); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao inserir gender no BigQuery: %w", err)
	}

	if err := socialClassInserter.Put(ctx, SociaClassRows); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao inserir social_class no BigQuery: %w", err)
	}

	if err := targetInserter.Put(ctx, TargetRows); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao inserir target no BigQuery: %w", err)
	}

	if err := geodataInserter.Put(ctx, GeodataRows); err != nil {
		return fmt.Errorf("[service/decode_csv] Erro ao inserir geodata no BigQuery: %w", err)
	}
