...
func Load() Config {
	...
	return Config{
		...
		GCSBucket:      getEnv("GCS_BUCKET", "kafka-backup-eletromidia"),
		GCSCredentials: getEnv("GCS_CREDENTIALS", ""),
		GCSBasePath:    getEnv("GCS_BASE_PATH", "kafka-backup"),
...
func (c Config) Validate() error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("variaveis de ambiente obrigatorias ausentes: %s", strings.Join(missing, ", "))
	}
	return nil
}
